...
1. Create a base dataset and join all relevant tables
  * `complete_joined_dataset`
----------------------------------------------------*/

DROP TABLE IF EXISTS complete_joined_dataset;
CREATE TEMP TABLE complete_joined_dataset AS
SELECT
  rental.customer_id,
  inventory.film_id,
  film.title,
...
DROP TABLE IF EXISTS actor_joined_dataset;
CREATE TEMP TABLE actor_joined_dataset AS
SELECT
  rental.customer_id,
  rental.rental_id,
  rental.rental_date,
...
SELECT DISTINCT
  actor_joined_dataset.film_id,
  actor_joined_dataset.actor_id,
  -- keep title in dataset as we need it for final recommendation text
  actor_joined_dataset.title,
  film_counts.rental_count
FROM actor_joined_dataset
LEFT JOIN film_counts
  ON actor_joined_dataset.film_id = film_counts.film_id;
