...
--- (total unique films recommended divided by total available films)
...
--- rounded to the nearest 2 decimal places? Hint: be careful of your data types!

SELECT
  ROUND(AVG(percentile::NUMERIC), 2)
FROM first_category_insights;

--- 7. What is the cumulative distribution of the top 5 percentile values 
--- for the top category from the first_category_insights table rounded to 
--- the nearest round percentage?
...
SELECT
PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY total_percentage)
...
--- 10. What was the average number of films watched by each 
--- customer rounded to the nearest whole number?

SELECT
  ROUND(AVG(total_count)) AS avg_count
FROM total_counts

--- 11. What is the top combination of top 2 categories 
--- and how many customers if the order is relevant (e.g. 
--- Horror and Drama is a different combination to Drama and Horror)
...
--- 13. How many films on average had customers already seen that 
--- feature their favourite actor rounded to closest integer?

SELECT
  ROUND(AVG(rental_count))
FROM top_actor_counts;

--- 14. CHALLENGE: What is the most common top categories combination 
--- if order was irrelevant and how many customers have this combination? 
...
FROM (
  SELECT
    LEAST(cat_1, cat_2) AS cat_1,
    GREATEST(cat_1, cat_2) AS cat_2,
    customer_id
  FROM
    final_data_asset
...
SELECT
  CASE WHEN category_1 < category_2 THEN category_1 ELSE category_2 END AS cat_1,
  CASE WHEN category_1 < category_2 THEN category_2 ELSE category_1 END AS cat_2,
  COUNT(*) AS customer_count
FROM
  final_data_asset
GROUP BY
